...
## Phase 3.5: Database Evolution & Persistence Strategy {#phase-3-5}

### Domain ↔ Persistence Split
- **Domain models** (`Game`, `Player`, `Deck`, `Card`) stay focused on business rules (methods like `dealCards`, `shuffle`, `addDeck`).
- **Persistence entities** mirror database tables and relationships (e.g., `GameEntity`, `PlayerEntity`). They carry JPA annotations such as `@Entity`, `@Table`, `@ManyToOne`.
- **Mappers** translate between entity and domain object when services load or persist data. This keeps business logic testable without a database and lets persistence evolve independently.

### Spring Data JPA Repositories
- Repositories become interfaces (e.g., `interface GameRepository extends JpaRepository<GameEntity, UUID>`).
- Spring generates the implementation at runtime: CRUD operations, derived queries (`findByGameIdAndName`), and `@Query` SQL/JPQL snippets.
- Benefits:
  - **Testing:** swap in mocks/stubs for services (no PostgreSQL required in unit tests).
  - **Consistency:** transactions, locking, optimistic concurrency via JPA features.
  - **Extensibility:** add projections, pagination, custom queries as the product grows.

### Workflow with PostgreSQL
1. Controller calls service with DTO request.
2. Service fetches entities via repository (optionally using `@EntityGraph` to eager-load relationships).
3. Service maps entities → domain objects, executes domain logic (producing “new state”).
4. Service maps updated domain state back into entities and calls `repository.save`.
5. Spring Data JPA issues SQL statements under the hood (INSERT/UPDATE/DELETE).
6. Service returns DTOs to controller for the HTTP response.
...
public interface GameRepository extends JpaRepository<GameEntity, UUID> {
    ...
    Optional<GameEntity> findWithPlayersById(UUID id);
}
```
...
where g.id = ?;
```
`@EntityGraph` avoids N+1 queries by fetching related players in the same round trip.

### Schema Sketch (PostgreSQL)
```
games(id uuid pk, created_at timestamptz, updated_at timestamptz)
decks(id uuid pk, created_at timestamptz, added boolean)
game_decks(game_id uuid fk, deck_id uuid fk, primary key(game_id, deck_id))
players(id uuid pk, game_id uuid fk, name text, hand_value int, hand_size int, unique(game_id, name))
cards(id bigserial pk, deck_id uuid fk, game_id uuid fk,
      owner_player_id uuid fk, suit text, face_value text,
      value int, display_name text, position int, dealt boolean)
```
- Tracks shoe order via `position`, ownership via `owner_player_id`, and supports analytics/querying undealt cards.
- Foreign keys preserve referential integrity; migrations (Flyway/Liquibase) manage future schema changes.

### Testing Strategy with Persistence
- **Unit tests:** mock repositories to supply domain objects without hitting the DB.
- **Integration tests:** use real Spring Data repositories + Testcontainers/PostgreSQL to validate mappings and transactions.
- **Transactional boundaries:** wrap multi-step operations (`dealCards`, `addDeckToGame`) in `@Transactional` to ensure atomic updates.

This narrative demonstrates how the prototype’s repository layer can evolve into a production-ready PostgreSQL solution while keeping domain logic clean and testable.
